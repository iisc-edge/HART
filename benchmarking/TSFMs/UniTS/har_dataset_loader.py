...
---------------------
Single script that knows the structure of all 18 HAR test datasets in Test_set_1.
Loads any dataset, applies sliding windows of 300 samples (no overlap),
and returns a df_cleaned DataFrame where each row is one window —
matching exactly the format used in tspulse_daphnet.ipynb.

Usage:
    from har_dataset_loader import load_dataset, DATASET_CONFIGS, TEST_SET_DIR
    ...
    from har_dataset_loader import load_all_datasets
...
# ─────────────────────────────────────────────────────────────────────────────
# Path to the Test_set_1 folder
# ─────────────────────────────────────────────────────────────────────────────
TEST_SET_DIR = "/media/user/DATA21/Time_Series_Foundation_Models/HAR/HAR_New/test_set"
...
DATASET_CONFIGS = {
    ...
    "capture24": {
        "file": "capture24_30hz_full_test.csv",
...
# ─────────────────────────────────────────────────────────────────────────────
# Core windowing function — matches the logic in tspulse_daphnet.ipynb exactly
# ─────────────────────────────────────────────────────────────────────────────
...
def load_dataset(
    ...
    """
    Load a HAR test dataset, apply non-overlapping windows of `window_length`
    samples, and return a df_cleaned DataFrame where each row is one window.
    ...
    window_length : int
        Number of raw samples per window. Default 300 (= 10 s @ 30 Hz).
    test_set_dir : str
        Path to the folder containing the CSV files.
    drop_incomplete_windows : bool
        If True (default), drop windows that have fewer than window_length rows
        (i.e. trailing samples that don't fill a full window).

    Returns
    -------
    pd.DataFrame
        df_cleaned — one row per window, ready to pass into
        tsppulse_prepare_dataset() / ClassificationDFDataset.
    """
    ...
    csv_path = os.path.join(test_set_dir, cfg["file"])
    ...
    # Assign a global window_id based on row index (non-overlapping windows)
    df["window_id"] = (df.index // window_length) + 1

    # Keep only full windows if requested
    if drop_incomplete_windows:
        window_sizes = df.groupby("window_id").size()
        full_windows = window_sizes[window_sizes == window_length].index
        df = df[df["window_id"].isin(full_windows)]

    print(f"[{dataset_name}] Windows: {df['window_id'].nunique()} (window_length={window_length})")

    # Build df_cleaned — one row per window (same structure as tspulse_daphnet.ipynb)
    rows = []
    for wid in df["window_id"].unique():
        w = df[df["window_id"] == wid]
        row = {}
        for col in signal_cols:
            row[col] = w[col].reset_index(drop=True)
        row[subject_col] = w[subject_col].iloc[0]
        row[label_col]   = w[label_col].value_counts().index[0]  # majority label in window
        row["window_id"] = wid
        rows.append(pd.DataFrame([row]))

    df_cleaned = pd.concat(rows).reset_index(drop=True)
